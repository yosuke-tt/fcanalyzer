...
def e0_bsp(t, e0, einf, alpha, k_exp,k_coeff,integral_range):
    beta_bsp_=beta_bsp(t, k_exp, alpha,integral_range)
    return k_coeff*(einf*beta_einf(k_exp,integral_range)+(e0-einf)*beta_bsp_) 

def e1_bsp(t, e1,einf,alpha,k_exp,k_coeff,integral_range, t_dash=1/50000):
    beta_bsp_=beta_bsp(t, k_exp, alpha,integral_range)
    return k_coeff*(einf*beta_einf(k_exp,integral_range)+(e1-einf)*((t_dash+t)**(1-alpha+k_coeff))*beta_bsp_) 
...
def bsp_stress_relaxation_for_e0(t, 
    ...
    if len(t_app)>0:
        bsp_app = bsp_beta_for_e0(t_app,0,t_app, e0, einf, alpha, k_exp_app, k_coeff_app)

    if len(t_sr)>0:
        bsp_app_in_sr = bsp_beta_for_e0(t_sr,0,t_app, e0, einf, alpha, k_exp_app, k_coeff_app)
        bsp_sr = bsp_beta_for_e0(t_sr,t_trig, t_sr, e0, einf, alpha, k_exp_srs, k_coeff_srs)
        bsp_sr+=bsp_app_in_sr
    bsp=np.append(bsp_app,bsp_in_sr)
    return bsp

def bsp_stress_relaxation_for_e1(t, e1,einf,alpha,t_trig,
                                 k_exp_app,k_coeff_app,k_exp_srs=(0,0),k_coeff_srs=(0,0)):
    t_app = t[t_trig>=t]
    t_sr = t[t_trig<t]
    bsp_app,bsp_in_sr=[],[]
    if len(t_app)>0:
        bsp_app = bsp_beta_for_e1(t_app,0,t_app, e1, einf, alpha, k_exp_app, k_coeff_app)
    if len(t_sr)>0:
        bsp_app_in_sr = bsp_beta_for_e1(t_sr, 0, t_app, e1,einf,alpha,k_exp_app,k_coeff_app)
        bsp_sr = bsp_beta_for_e1(t_sr, t_trig, t_sr, e1, einf, alpha, k_exp_srs, k_coeff_srs)
...
def bsp_beta_for_e0(time, integral_start, integral_end, 
                    e0, einf, alpha,
                    k_exp, k_coeff):
    bsp_beta = np.sum([e0_bsp(time,
                               e0, einf, alpha,
                               k_exp_, k_coeff_,
...
def bsp_beta_for_e1(time, integral_start, integral_end, 
                    e1, einf, alpha,
                    k_exp, k_coeff):
    bsp_beta = np.sum([e1_bsp(time,
                               e1, einf, alpha,
                               k_exp_, k_coeff_,
                               integral_range=(integral_start,integral_end)
                               )
